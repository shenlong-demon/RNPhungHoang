...
import { Brand, DataContextResult, STATUS, useDataContext } from '@src/business';
...
export const UpdateBrandScreen = () => {
  ...
  return (
    <View style={{ flex: 1 }}>
      {/* Brand form */}
      <View>
        <TextInput placeholder="Brand name" value={name} onChangeText={(text) => setName(text)} />
        <TextInput
          placeholder="Status (1: active, 2: inactive)"
          value={status.toString()}
          onChangeText={(text) => setStatus(parseInt(text))}
        />
        <Button title={selectedBrand ? 'Update' : 'Create'} onPress={handleSaveBrand} />
      </View>
